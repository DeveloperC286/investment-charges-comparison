...
def main(argv):
    ...
    if not path.isfile(args.input):
        print(
            "The input CSV file '{}' does not exist or is not a file.".format(
                args.input))
        sys.exit(2)
                ...
                investment = (
                    projection(
                        float(
                            ...
                            row[6]), float(
                                row[7])))
                plt.plot(range(0, int(row[3]) + 1),
                         investment, color=str(row[1]).strip())
                temp_label = "{} Total : {}".format(str(row[0]), str(
                    locale.currency(investment[len(investment) - 1], grouping=True)))
                temp_patch = mpatches.Patch(
                    color=str(row[1]).strip(), label=temp_label)
